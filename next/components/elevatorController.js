...
export default function ElevatorController() {
  const contruoller = useRef();

  const ControllOFF = () => {
    contruoller.currect.src = "http://192.168.88.140/RELAY=OFF";

    if (
      document.getElementById("Controller").src ==
      "http://192.168.88.140/RELAY=OFF"
    ) {
  ...
  return (
    <div>
      ...
      <iframe
        ...
        width="560"
        height="315"
        src="http://192.168.88.87:81/stream"
        name="iframe_b"
        frameborder="0"
        allowfullscreen
      ></iframe>
      ...
      <nav className={styles.nav}>
        <iframe
          ...
          width="0"
          height="0"
          src="http://192.168.0.115/RELAY=OFF"
          frameborder="0"
          allowfullscreen
        ></iframe>
        <br />
        <center>
          <p>電梯控制器</p>
          <button id="buttonON" onClick={ControllON} className={styles.button}>
            ON
          </button>
          <button
            id="buttonOFF"
